...
export function UserRegisterForm({ className, ...props }: UserAuthFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof registerFormSchema>) {
    ...
    console.log("Register results in Register form: ", userInfo)

    if (userInfo.error) {
      if (userInfo.error === "Email or Username already exists"){
      ...
      {registerError && (
        <Card className="border-1 border-red-500 bg-black p-2">
          ...
          <CardBody className="text-red-500">
            This email or username already has an associated account, please use another email. 
          </CardBody>
        </Card>
      )}
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
            <Input type="text" label="Username" labelPlacement="inside" {...register("username")}/>
            {errors.username && (
            <span className="flex text-red-500">{errors.username?.message}</span>
            )}
            ...
            <Input  id="password" type="password" label="Confirm Password" labelPlacement="inside" {...register("confirmPassword")}/>
            {errors.confirmPassword && (
                <span className="flex text-red-500">{errors.confirmPassword?.message}</span>
            )}
