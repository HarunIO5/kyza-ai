...
export default function HyperRealisticVideoForm() {
  ...
  const onSubmit = async (
    ...
    const response = await fetch("/api/animatediff", {
      ...
      body: JSON.stringify({
        prompt: `Generate hyper-realistic visuals, focus on photorealistic textures, cinematic lighting and depth, ultra-detailed environments, real-world accuracy and motion. Based of the following prompt: ${values.prompt}`,
  ...
  useEffect(() => {
    async function firstGeneration({ prompt }: { prompt: string }) {
      ...
      const response = await fetch("/api/animatediff", {
        ...
        body: JSON.stringify({
          prompt: `Generate hyper-realistic visuals, focus on photorealistic textures, cinematic lighting and depth, ultra-detailed environments, real-world accuracy and motion. Based of the following prompt: ${prompt}`,
          // negative:
          //   "nsfw, ng_deepnegative_v1_75t, badhandv4, worst quality, low quality, normal quality, lowres, watermark, monochrome",
          // scale: 7.5,
            ...
            <div className="flex-grow w-full">
              ...
              >
                <div className="w-full mb-4">
                  <Textarea
                    type="text"
                    label="Describe your hyper-realistic video"
                    disabled={isLoading}
                    value={prompt}
