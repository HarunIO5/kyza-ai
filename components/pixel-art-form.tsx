...
export default function PixelArtForm() {
  ...
  const onSubmit = async (
    ...
    const response = await fetch("/api/stableDiffuse", {
      ...
      body: JSON.stringify({
        prompt: `Create pixelated retro designs, use 8-bit or 16-bit color palettes, focus on simplicity and blocky shapes, retro video game aesthetics, chiptune-era visual style. Based on the following prompt: ${values.prompt}`,
  ...
  useEffect(() => {
    async function firstGeneration({ prompt }: { prompt: string }) {
      ...
      const response = await fetch("/api/stableDiffuse", {
        ...
        body: JSON.stringify({
          prompt: `Create pixelated retro designs, use 8-bit or 16-bit color palettes, focus on simplicity and blocky shapes, retro video game aesthetics, chiptune-era visual style. Based on the following prompt: ${prompt}`,
          // scale: scale,
          // spees: speed,
          aspect_ratio: ratio,
          // email: session?.user?.email!,
              ...
              >
                <div className="w-full mb-4">
                  <Textarea
                    ...
                    placeholder="Cyberpunk street vendor with neon sign, rain effects, 64×64, night scene, purple and teal color palette, side view, animated steam rising from food stall."
                    className="pb-4"
                    {...register("prompt", {
                      onChange: (e) => {
